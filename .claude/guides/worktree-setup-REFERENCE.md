# Worktree Setup Guide (Full Reference)

> **Full Reference** - Complete bash scripts and troubleshooting
> **Quick Reference**: @.claude/guides/worktree-setup.md

## Purpose

Complete worktree setup for isolated development environments.

**Usage**: `/02_execute --wt` flag enables worktree mode

## Overview

Worktree mode allows working on multiple branches simultaneously in isolated directories. Each worktree has its own working directory but shares the same Git object database.
...
Main repo + worktree-local pointers:
