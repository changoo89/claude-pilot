#!/bin/bash
# Test: GPT delegation in /91_document
# TS-4: /91_document should trigger GPT Architect delegation for complex documentation tasks
...
COMMAND_FILE="$PROJECT_ROOT/.claude/commands/91_document.md"

echo "=== TS-4: GPT Delegation Trigger in /91_document ==="
...
if grep -i "Delegate to GPT Architect" "$COMMAND_FILE" | grep -qi "context.md\|component" > /dev/null 2>&1; then
...
echo ""
echo "=== All tests PASSED for /91_document ==="
exit 0
