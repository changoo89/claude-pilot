...
if [ "$UNCHECKED" -gt 0 ]; then
    ...
    # Task: subagent_type: claude-pilot:validator, prompt: "Analyze unchecked TODOs in $PLAN_PATH. Identify blockers and attempt resolution. For each TODO: 1) Check if already completed (code exists), 2) Attempt to complete if trivial, 3) Mark [x] if resolved, 4) Report blockers requiring user input."
